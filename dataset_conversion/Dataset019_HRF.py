...
def copy_random_images(source_dir, target_dir, n, valid_extensions=None):
    """
    从源目录随机挑选 n 张图片复制到目标目录。

    :param source_dir: 输入目录路径
    :param target_dir: 输出目录路径
    :param n: 需要复制的图片数量
    :param valid_extensions: 图片后缀列表 (默认为常见图片格式)
    """
    # 1. 设置默认支持的图片格式
    ...
    if not os.path.exists(source_dir):
        print(f"错误：输入目录 '{source_dir}' 不存在。")
        return
    ...
    # 3. 处理 n 大于实际图片数量的情况
    if n > total_images:
        print(f"警告：请求复制 {n} 张，但只有 {total_images} 张。将复制所有图片。")
        n = total_images
    ...
    for file_name in selected_images:
        ...
        try:
            # copy2 会保留文件的元数据（如时间戳）
            shutil.copy2(src_path, dst_path)
            count += 1
        except Exception as e:
            print(f"复制文件 {file_name} 失败: {e}")

    print(f"成功随机复制了 {count} 张图片到 '{target_dir}'。")
...
def rename_fives_inplace(target_dir):
    ...
    for i, filename in enumerate(raw_files, start=1):
        ...
        if target_dir.endswith('imagesTr'):
            new_filename = f"CHASE-DB1_{i:03d}_0000.tif"
        elif target_dir.endswith('labelsTr'):
            new_filename = f"CHASE-DB1_{i:03d}.tif"
...
if __name__ == '__main__':
    # ================= 配置路径 =================
    imagesTr = "./nnUNet_raw/Dataset019_HRF/imagesTr"
    labelsTr = "./nnUNet_raw/Dataset019_HRF/labelsTr"
    
    imagesTs = "./nnUNet_raw/Dataset019_HRF/imagesTs"
    labelsTs = "./nnUNet_raw/Dataset019_HRF/labelsTs"

    predictions = "./nnUNet_raw/Dataset019_HRF/predictions/nnLoGoNet"
    masksTs = "./nnUNet_raw/Dataset019_HRF/masksTs"
    
    copy_random_images(imagesTr, imagesTs, 20)
