...
def clDice(v_p, v_l):  
    """计算clDice指标"""  
    v_p = (v_p > 0.5).astype(bool)
    v_l = (v_l > 0.5).astype(bool)
    ...
    if sum_pred == 0 and sum_ref == 0:
        return 1.0
    
    if sum_pred == 0 or sum_ref == 0:
        return 0.0
    ...
    if len(v_p.shape)==2 or len(v_p.shape)==3:  
        ...
        # 防御性编程：如果维度既不是2也不是3（例如是4维 Batch），抛出明确错误而不是报 UnboundLocalError
        raise ValueError(f"clDice 不支持输入数据的维度: {v_p.shape}, 仅支持 2D 或 3D") 
