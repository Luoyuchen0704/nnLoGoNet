...
class MambaLayer(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2):
        super().__init__()
        self.dim = dim
        # 这里使用简化版的2D Mamba逻辑：将特征图展平为序列进行扫描
        # 在实际工程中，建议使用类似 VMamba 的横向、纵向双向扫描策略
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x_flat = x.permute(0, 2, 3, 1).view(B, H * W, C) # [B, N, C]
        x_norm = self.norm(x_flat)
        x_mamba = self.mamba(x_norm)
        out = x_mamba.view(B, H, W, C).permute(0, 3, 1, 2)
        return out

class MambaGlobalBlock(nn.Module):
    # 确保参数列表与调用处匹配
    def __init__(self, in_dim, dim, mlp_ratio=4., drop=0., drop_path=0., 
                 num_heads=None, sr_ratio=1, **kwargs): # 增加 num_heads 和 kwargs
        super().__init__()
        self.proj = nn.Conv2d(in_dim, dim, kernel_size=3, padding=1)
        self.norm1 = nn.GroupNorm(1, dim)
        
        # 这里的 MambaLayer 应该替换为你实际实现的 Mamba 核心
...
class MambaNet2D(nn.Module):
    def __init__(self, in_channel, num_classes, deep_supervised, layer_channel=[16, 32, 64, 128, 256], global_dim=[8, 16, 32, 64, 128], num_heads=[1, 2, 4, 8], sr_ratio=[8, 4, 2, 1], dropout_op=None, dropout_op_kwargs=None):
        super(MambaNet2D, self).__init__()
        ...
        self.encoder1_l1_global = MambaGlobalBlock(layer_channel[0], global_dim[0], sr_ratio=sr_ratio[0])

        self.encoder1_l2_local = OPE(layer_channel[1], layer_channel[2])
        self.encoder1_l2_global = MambaGlobalBlock(layer_channel[1], global_dim[1], sr_ratio=sr_ratio[1])

        self.encoder1_l3_local = OPE(layer_channel[2], layer_channel[3])
        self.encoder1_l3_global = MambaGlobalBlock(layer_channel[2], global_dim[2], sr_ratio=sr_ratio[2])

        
        self.encoder1_l4_local = OPE(layer_channel[3], layer_channel[4])
        self.encoder1_l4_global = MambaGlobalBlock(layer_channel[3], global_dim[3], sr_ratio=sr_ratio[3])
...
class nnUNetTrainer_MambaNet(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
    ...
    def build_network_architecture(architecture_class_name,
        ...
        model = MambaNet2D(in_channel=num_input_channels, num_classes=num_output_channels, deep_supervised=enable_deep_supervision)
        model.apply(InitWeights_He(1e-2))
        return model

class nnUNetTrainer_MambaNet_SRL(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
