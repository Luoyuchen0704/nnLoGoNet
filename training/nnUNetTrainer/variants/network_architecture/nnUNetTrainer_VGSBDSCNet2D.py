...
import nnUNetTrainer_WDSCNet2D
...
class global_block(nn.Module):
    def __init__(self, in_dim, dim, num_heads, pool_size=3, mlp_ratio=4., drop=0., drop_path=0., sr_ratio=1):
        ...
        # Conv分支
        self.norm1_conv = GroupNorm(dim)
        self.conv_branch = nn.Sequential(
            nnUNetTrainer_WDSCNet2D.MultiView_DSConv(
            in_channels=dim,
            out_channels=dim,
    ...
    def forward(self, x):
        x = self.proj(x)
        # 并行处理两个分支
        trans_feat = self.attn(self.norm1(x))
        conv_feat = self.conv_branch(self.norm1_conv(x))
        
        # 通过可学习参数融合
        fused_feat = self.alpha * trans_feat + self.beta * conv_feat
...
class nnUNetTrainer_VGSBDSCNet2D(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
