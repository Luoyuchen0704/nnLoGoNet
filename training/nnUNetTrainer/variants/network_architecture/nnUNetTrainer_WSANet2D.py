...
class WNet2D(nn.Module):
    ...
    def forward(self, x):
        ...
        # Bottleneck mix conv and attention
        x_l1 = x_e1_l4_local
        ...
        # TSA l-latent space feature
        x_l2_pam = self.pam(x_l2)
        
        # GSA
        x_l2_pos = self.pos(x_l2)
        x_l2 = x_l2 + x_l2_pos
        x_l2_sdpa = self.sdpa(x_l2)
        
        # 融合
        x_l2 = x_l2_sdpa + x_l2_pam
...
class nnUNetTrainer_WSANet2D(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
